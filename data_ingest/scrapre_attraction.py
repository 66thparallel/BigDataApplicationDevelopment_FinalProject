...
def parse(session, url):
    ...
    rating = soup.select(".overallRating ")[0].text
    #taplc_detail_filters_ar_responsive_0 > div > div.collapsible.is-shown-at-tablet > div > div.ui_columns.filters > div
    rating = float(rating)
    about = soup.select("#taplc_location_detail_about_card_0 > div > div > div:nth-child(2) > span:nth-child(1)")[0].text
    #component_26 > div > div:nth-child(2) > span:nth-child(1)
    #component_26 > div > div:nth-child(2)
    #component_29 > div > div:nth-child(2) > span:nth-child(1)
    title = soup.select("#HEADING")[0].text
    return (title, rating, about)
